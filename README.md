...
This is the driver-specific `codex/stateless-batched-eval-550` branch. On a four-GPU node with kernel driver `550.163.01`, use `orchestration/run_4gpu_fastest_550.sh`; it fixes the matching userspace rendering stack, four policy servers, and sixteen environment workers. The separate `codex/stateless-batched-eval-570` branch provides `run_4gpu_fastest_570.sh`. The launchers intentionally do not auto-select a driver ABI.
